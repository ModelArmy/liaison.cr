...
describe "cross-protocol handoff" do
  ...
  it "carries a Chat Completions reply into an Anthropic request" do
    ...
    reply = Liaison::Protocol::ChatCompletions::Exporter.new(mapper.calls)
      .export_reply(RF::CHAT_TEXT)

    session = continued(reply)
    request, report = Liaison::Protocol::Anthropic::Mapper.new
  ...
  it "does not let the answering provider influence the next mapping" do
    ...
    session = continued(reply)
    theirs, _ = Liaison::Protocol::Gemini::Mapper.new.map(session, "gemini-2.0-flash")
    ...
    ours, _ = Liaison::Protocol::Gemini::Mapper.new.map(naive, "gemini-2.0-flash")
  ...
  it "pairs a tool call read from Gemini with a result rendered on Anthropic" do
    gemini = Liaison::Protocol::Gemini::Mapper.new
    reply = Liaison::Protocol::Gemini::Exporter.new(gemini.calls)
    ...
    rendered = request.messages[1].content.select(Liaison::Protocol::Anthropic::Wire::ToolUseBlock)
    results = request.messages[2].content.select(Liaison::Protocol::Anthropic::Wire::ToolResultBlock)
  ...
  it "rebinds provisional Gemini call keys on the next map" do
    ...
    session = continued(reply)
    first, _ = Liaison::Protocol::Gemini::Mapper.new.map(session, "gemini-2.0-flash")
    second, _ = Liaison::Protocol::Gemini::Mapper.new.map(session, "gemini-2.0-flash")
  ...
  it "does not ask a second provider to dispatch a call the first already ran" do
    ...
    reply = Liaison::Protocol::Anthropic::Exporter.new(anthropic.calls)
      .export_reply(RF::ANTHROPIC_SERVER_TOOL)

    session = continued(reply)
    _, report = Liaison::Protocol::ChatCompletions::Mapper.new
  ...
  it "survives a detour through a protocol that cannot carry its signature" do
    ...
    # session.
    request, _ = Liaison::Protocol::Anthropic::Mapper.new
      .map(session, "claude-sonnet-4-6", C::Policy::Lenient)

    thinking = request.messages[1].content
      .select(Liaison::Protocol::Anthropic::Wire::ThinkingBlock)
  ...
  it "accepts replies from all four protocols in one session" do
    ...
    chat = Liaison::Protocol::ChatCompletions::Mapper.new
    session << Liaison::Protocol::ChatCompletions::Exporter.new(chat.calls)
      .export_reply(RF::CHAT_TEXT)
    session << M::Message.user("And the deepest ocean?")

    responses = Liaison::Protocol::Responses::Mapper.new
    session << Liaison::Protocol::Responses::Exporter.new(responses.calls)
      .export_reply(RF::RESPONSES_TEXT)
    session << M::Message.user("And the longest river?")

    claude = Liaison::Protocol::Anthropic::Mapper.new
    session << Liaison::Protocol::Anthropic::Exporter.new(claude.calls)
      .export_reply(RF::ANTHROPIC_TEXT)
    session << M::Message.user("And the largest desert?")

    google = Liaison::Protocol::Gemini::Mapper.new
    session << Liaison::Protocol::Gemini::Exporter.new(google.calls)
    ...
    chat_request, chat_report = Liaison::Protocol::ChatCompletions::Mapper.new
      .map(session, "llama3.2", C::Policy::Lenient)
    claude_request, claude_report = Liaison::Protocol::Anthropic::Mapper.new
      .map(session, "claude-sonnet-4-6", C::Policy::Lenient)
    gemini_request, gemini_report = Liaison::Protocol::Gemini::Mapper.new
