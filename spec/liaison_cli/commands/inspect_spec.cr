...
require "../../../src/liaison_cli/commands/list"
require "../../../src/liaison_cli/commands/show"
...
# under `spec/liaison_cli/commands/`, there is no Wiretap here and nothing to
...
private def with_sandbox(&) : Nil
  tmp = File.join(Dir.tempdir, "liaison-cli-inspect-spec-#{Random.rand(1_000_000)}")
...
describe Liaison::Cli::Commands::List do
  it "says so plainly when there is nothing to list" do
    with_sandbox do
      _, warned = captured { Liaison::Cli::Commands::List.run([] of String) }
  ...
  it "lists each session with its turn count and last deployment" do
    with_sandbox do
      ...
      printed, _ = captured { Liaison::Cli::Commands::List.run([] of String) }
  ...
  it "counts a tool exchange as part of the turn that prompted it" do
    with_sandbox do
      ...
      Liaison::Cli::Sessions.snapshot("brisk-comet", session, "ollama")
  ...
  it "skips a session folder with no saved turns rather than failing" do
    with_sandbox do
      ...
      Dir.mkdir_p(Liaison::Cli::Sessions.path_for("empty-husk"))
  ...
  it "skips entries that are not session ids at all" do
    with_sandbox do
      seed("brisk-comet", "ollama", "intact")
      File.write(File.join(Liaison::Cli::Sessions.folder, ".DS_Store"), "junk")
      Dir.mkdir_p(File.join(Liaison::Cli::Sessions.folder, ".hidden-thing"))
...
describe Liaison::Cli::Commands::Show do
  it "prints the system prompt and every turn" do
    with_sandbox do
      seed("brisk-comet", "ollama", "what is the tallest mountain?")

      printed, _ = captured { Liaison::Cli::Commands::Show.run(["brisk-comet"]) }
  ...
  it "renders blocks that are not text, rather than dropping them" do
    with_sandbox do
      ...
      Liaison::Cli::Sessions.snapshot("brisk-comet", session, "gemini")
  ...
  it "lists the append-only turn history under --snapshots" do
    with_sandbox do
      ...
      printed, _ = captured { Liaison::Cli::Commands::Show.run(["brisk-comet", "--snapshots"]) }
  ...
  it "emits the archive exactly as stored under --json" do
    with_sandbox do
      ...
      printed, _ = captured { Liaison::Cli::Commands::Show.run(["brisk-comet", "--json"]) }

      printed.chomp.should eq(Liaison::Cli::Sessions.latest_archive("brisk-comet"))
  ...
  it "refuses two flags that ask for different things" do
    with_sandbox do
      ...
      expect_raises(ArgumentError, /different things/) do
        Liaison::Cli::Commands::Show.run(["brisk-comet", "--snapshots", "--json"])
      end
    end
  end
  ...
  it "names the session it could not find" do
    with_sandbox do
      expect_raises(Liaison::Cli::SessionError, /no-such-session/) do
        Liaison::Cli::Commands::Show.run(["no-such-session"])
      end
    end
  end
end
