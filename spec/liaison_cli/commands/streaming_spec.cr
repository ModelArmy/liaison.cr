...
private STREAM_START    = "liaison_cli_stream_start"
private STREAM_CONTINUE = "liaison_cli_stream_continue"
...
# `$LIAISON_HOME` rather than `Dir.cd`.
private def with_sandbox(&) : Nil
  tmp = File.join(Dir.tempdir, "liaison-cli-streaming-spec-#{Random.rand(1_000_000)}")
...
private def printed : String
  Liaison::Cli::Output.stream.to_s
end

private def warned : String
  Liaison::Cli::Output.error_stream.to_s
end
...
private def saved_reply(id : String) : M::Message
  Liaison::Cli::Sessions.latest(id).messages.last
end

describe "the CLI's streamed turn" do
  describe "liaison start --stream" do
    it "saves the session a non-streamed run would have saved" do
      ...
      with_sandbox do
        Wiretap.intercept(STREAM_START) do
          Liaison::Cli::Commands::Start.run(["ollama", PROMPT, SYSTEM, "--stream"])
        end

        session = Liaison::Cli::Sessions.latest(only_session)
  ...
  describe "liaison continue --stream" do
    it "appends a streamed turn to a session that already exists" do
      with_sandbox do
        ...
        Wiretap.intercept(STREAM_CONTINUE) do
          Liaison::Cli::Commands::Continue.run([id, "And the second tallest?", "--stream"])
        end
    ...
    it "prints only the second answer, not the conversation it continued" do
      ...
      with_sandbox do
        ...
        Liaison::Cli::Output.stream.as(IO::Memory).clear
    ...
    it "declines to stream into a redirected stdout, whatever the config says" do
      ...
      with_sandbox do
        Wiretap.intercept("liaison_cli_start_ollama") do
          Liaison::Cli::Commands::Start.run(["ollama", PROMPT, SYSTEM])
        end
