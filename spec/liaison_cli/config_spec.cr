...
# site.
private def deployment_with(*lines : String) : Liaison::Cli::Deployment
  yaml = String.build do |io|
    io << "servers:\n  ollama:\n    protocol: chat_completions\n    url: http://localhost:11434\n"
    io << "deployments:\n  qwen:\n    server: ollama\n    model: qwen3.8\n"
    lines.each { |line| io << "    " << line << "\n" }
  end
  Liaison::Cli::Config.from_yaml(yaml).deployment("qwen")
end

describe Liaison::Cli::Config do
  describe "defaults" do
    base = "servers:\n  ollama:\n    protocol: chat_completions\n    url: http://localhost:11434\n" \
           "deployments:\n  qwen:\n    server: ollama\n    model: qwen3.8\n"

    it "is quiet and unstreamed when the block is absent" do
      defaults = Liaison::Cli::Config.from_yaml(base).defaults
    ...
    it "reads either key on its own" do
      config = Liaison::Cli::Config.from_yaml(base + "defaults:\n  streaming: true\n")
    ...
    it "reads both" do
      config = Liaison::Cli::Config.from_yaml(
        base + "defaults:\n  streaming: true\n  show_reasoning: true\n")
    ...
    it "refuses a value that is not a boolean, and names the key" do
      expect_raises(Liaison::Cli::ConfigError, /defaults.streaming/) do
        Liaison::Cli::Config.from_yaml(base + "defaults:\n  streaming: 1\n")
      end
    end

    it "refuses a defaults that is not a block" do
      expect_raises(Liaison::Cli::ConfigError, /not a block/) do
        Liaison::Cli::Config.from_yaml(base + "defaults: true\n")
      end
    end
  end
  ...
  describe ".from_yaml" do
    it "parses a server and a deployment pointing at it" do
      config = Liaison::Cli::Config.from_yaml(<<-YAML)
        servers:
          anthropic:
            protocol: anthropic
      ...
      d.protocol.should eq(Liaison::ProtocolKind::Anthropic)
    ...
    it "parses an azure block and max_tokens_field on the server" do
      ...
      s.max_tokens_field.should eq(Liaison::Protocol::ChatCompletions::Wire::MaxTokensField::MaxCompletionTokens)
    end
    ...
    it "raises at parse time for an azure block with no api_version" do
      expect_raises(Liaison::Cli::ConfigError, /api_version/) do
        Liaison::Cli::Config.from_yaml(<<-YAML)
          servers:
            azure-alpha:
              protocol: chat_completions
    ...
    it "raises when 'servers' is missing entirely" do
      expect_raises(Liaison::Cli::ConfigError, /servers/) do
        Liaison::Cli::Config.from_yaml("default_deployment: anthropic")
      end
    end

    it "raises when 'deployments' is missing entirely" do
      expect_raises(Liaison::Cli::ConfigError, /deployments/) do
        Liaison::Cli::Config.from_yaml(<<-YAML)
          servers:
            ollama:
              protocol: chat_completions
    ...
    it "recognises the old flat format and says what to change" do
      expect_raises(Liaison::Cli::ConfigError, /format changed.*servers:/m) do
        Liaison::Cli::Config.from_yaml(<<-YAML)
          deployments:
            anthropic:
              protocol: anthropic
    ...
    it "raises naming the deployment when its server is not defined" do
      expect_raises(Liaison::Cli::ConfigError, /"qwen".*"typo-ollama".*not defined/) do
        Liaison::Cli::Config.from_yaml(<<-YAML)
          servers:
            ollama:
              protocol: chat_completions
    ...
    it "raises naming the server when 'protocol' is missing" do
      expect_raises(Liaison::Cli::ConfigError, /"anthropic".*protocol/) do
        Liaison::Cli::Config.from_yaml(<<-YAML)
          servers:
            anthropic:
              url: https://api.anthropic.com
    ...
    it "raises naming the server when 'url' is missing" do
      expect_raises(Liaison::Cli::ConfigError, /"anthropic".*url/) do
        Liaison::Cli::Config.from_yaml(<<-YAML)
          servers:
            anthropic:
              protocol: anthropic
    ...
    it "raises naming the deployment when 'model' is missing" do
      expect_raises(Liaison::Cli::ConfigError, /"haiku".*model/) do
        Liaison::Cli::Config.from_yaml(<<-YAML)
          servers:
            anthropic:
              protocol: anthropic
    ...
    it "raises on an unrecognised protocol" do
      expect_raises(Liaison::Cli::ConfigError, /unrecognised protocol "azure-native"/) do
        Liaison::Cli::Config.from_yaml(<<-YAML)
          servers:
            weird:
    ...
    it "raises on an unrecognised max_tokens_field" do
      expect_raises(Liaison::Cli::ConfigError, /unrecognised max_tokens_field "output_limit"/) do
        Liaison::Cli::Config.from_yaml(<<-YAML)
          servers:
            weird:
              protocol: chat_completions
  ...
  describe "model preferences" do
    ...
    it "parses each reasoning level" do
      deployment_with("reasoning: low").reasoning.should eq(Liaison::Reasoning::Effort::Low)
      deployment_with("reasoning: medium").reasoning.should eq(Liaison::Reasoning::Effort::Medium)
      deployment_with("reasoning: high").reasoning.should eq(Liaison::Reasoning::Effort::High)
      deployment_with("reasoning: xhigh").reasoning.should eq(Liaison::Reasoning::Effort::XHigh)
      deployment_with("reasoning: max").reasoning.should eq(Liaison::Reasoning::Effort::Max)
    end
    ...
    it "reads 'none' as Reasoning::Off, which is not the same as absent" do
      deployment_with("reasoning: none").reasoning.should be_a(Liaison::Reasoning::Off)
    end

    it "reads an integer as a token budget" do
      deployment_with("reasoning: 4096").reasoning.should eq(Liaison::Reasoning::Budget.new(4096))
    end
    ...
    it "explains itself when YAML turns a bare off into a boolean" do
      expect_raises(Liaison::Cli::ConfigError, /boolean.*write 'none'/) do
        deployment_with("reasoning: off")
      end
    end

    it "rejects a non-positive budget" do
      expect_raises(Liaison::Cli::ConfigError, /must be positive/) { deployment_with("reasoning: 0") }
    end

    it "rejects an unrecognised level" do
      expect_raises(Liaison::Cli::ConfigError, /unrecognised reasoning "ludicrous"/) do
    ...
    it "parses each retention mode" do
      deployment_with("reasoning_retention: all")
        .reasoning_retention.should eq(Liaison::Capability::ReasoningRetention::All)
      deployment_with("reasoning_retention: completed_turns")
        .reasoning_retention.should eq(Liaison::Capability::ReasoningRetention::CompletedTurns)
      deployment_with("reasoning_retention: none")
        .reasoning_retention.should eq(Liaison::Capability::ReasoningRetention::None)
    end

    it "rejects an unrecognised retention mode" do
      expect_raises(Liaison::Cli::ConfigError, /unrecognised reasoning_retention "sometimes"/) do
    ...
    it "carries both at once" do
      ...
      d.reasoning.should be_a(Liaison::Reasoning::Off)
      d.reasoning_retention.should eq(Liaison::Capability::ReasoningRetention::CompletedTurns)
    end
  end
  ...
  describe "#deployment" do
    it "raises listing the known deployment names when asked for an unknown one" do
      ...
      expect_raises(Liaison::Cli::ConfigError, /"azure-mini".*haiku/) { config.deployment("azure-mini") }
    end
  end
  ...
  describe "#provider_for" do
    ...
    it "names the Provider's Server for the server entry, not the deployment" do
      ...
      provider.adapter.class.should eq(Liaison::AnthropicAdapter)
    end
    ...
    it "builds an Azure Provider via Provider.for_azure" do
      ...
      provider.adapter.class.should eq(Liaison::AzureChatCompletionsAdapter)
    end
  end

  describe Liaison::Cli::ServerConfig do
    it "resolves credential from the named environment variable" do
      ENV["LIAISON_SPEC_TEST_KEY"] = "shh"
      begin
        s = Liaison::Cli::ServerConfig.new("x", Liaison::ProtocolKind::Anthropic, "https://example.com",
          credential_env: "LIAISON_SPEC_TEST_KEY")
        s.credential.should eq("shh")
      ensure
        ENV.delete("LIAISON_SPEC_TEST_KEY")
      end
    end

    it "is nil when credential_env is unset" do
      s = Liaison::Cli::ServerConfig.new("x", Liaison::ProtocolKind::ChatCompletions, "http://localhost:11434")
