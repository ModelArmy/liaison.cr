require "../spec_helper"
require "../../src/liaison_cli/progress"
...
describe Liaison::Cli::Progress do
  ...
  it "writes nothing at all when the stream is not a terminal" do
    sink = IO::Memory.new
    Liaison::Cli::Progress.while_waiting("thinking", sink) do
      sleep(Liaison::Cli::Progress::TICK * 3)
    end
  ...
  it "erases the line even when the block raises" do
    ...
    expect_raises(ArgumentError, "boom") do
      Liaison::Cli::Progress.while_waiting("thinking", sink) do
        sleep(Liaison::Cli::Progress::TICK * 2)
  ...
  it "reports elapsed seconds, not just motion" do
    sink = FakeTty.new
    Liaison::Cli::Progress.while_waiting("thinking", sink) do
      sleep(Liaison::Cli::Progress::TICK * 5)
    end
  ...
  it "picks up a label changed while running" do
    sink = FakeTty.new
    Liaison::Cli::Progress.while_waiting("thinking", sink) do |indicator|
      sleep(Liaison::Cli::Progress::TICK * 2)
  ...
  it "returns the block's value untouched" do
    sink = IO::Memory.new
    result = Liaison::Cli::Progress.while_waiting("thinking", sink) { {1, "two"} }
    result.should eq({1, "two"})
  end

  it "is safe to stop without having started" do
    Liaison::Cli::Progress.new("thinking", IO::Memory.new).stop
  end
end
