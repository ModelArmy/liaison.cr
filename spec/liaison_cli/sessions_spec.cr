...
# `Sessions.root` checks `$CWD/.liaison` *exists* before choosing it — it
# doesn't create one speculatively — so sandboxing means pre-creating an
# empty `.liaison` in a temp dir and `Dir.cd`-ing into it, not just changing
...
private def with_sandbox(&) : Nil
  tmp = File.join(Dir.tempdir, "liaison-cli-spec-#{Random.rand(1_000_000)}")
...
describe Liaison::Cli::Sessions do
  it "resolves root to $CWD/.liaison when it exists" do
    with_sandbox do
      Liaison::Cli::Sessions.root.should eq(File.join(Dir.current, ".liaison"))
    end
  end

  it "generates a short, memorable, unused id" do
    with_sandbox do
      id = Liaison::Cli::Sessions.generate_id
      id.should match(/\A[a-z]+-[a-z]+\z/)
      Liaison::Cli::Sessions.exists?(id).should be_false
    end
  end
  ...
  it "round-trips a snapshot and reports which deployment produced it" do
    with_sandbox do
      ...
      id = Liaison::Cli::Sessions.generate_id
      Liaison::Cli::Sessions.snapshot(id, session, "anthropic")

      Liaison::Cli::Sessions.exists?(id).should be_true
      Liaison::Cli::Sessions.latest_deployment(id).should eq("anthropic")

      restored = Liaison::Cli::Sessions.latest(id)
  ...
  it "keeps every snapshot rather than overwriting the last one" do
    with_sandbox do
      id = Liaison::Cli::Sessions.generate_id
      Liaison::Cli::Sessions.snapshot(id, M::Session.new, "anthropic")
      Liaison::Cli::Sessions.snapshot(id, M::Session.new, "azure-mini")

      Dir.children(Liaison::Cli::Sessions.path_for(id)).size.should eq(2)
      Liaison::Cli::Sessions.latest_deployment(id).should eq("azure-mini")
    end
  end
  ...
  it "orders snapshots written in the same millisecond by write order, not by name" do
    with_sandbox do
      id = Liaison::Cli::Sessions.generate_id
      Liaison::Cli::Sessions.snapshot(id, M::Session.new, "ollama")
      Liaison::Cli::Sessions.snapshot(id, M::Session.new, "ollama-responses")

      Liaison::Cli::Sessions.snapshots(id).size.should eq(2)
      Liaison::Cli::Sessions.latest_deployment(id).should eq("ollama-responses")

      other = Liaison::Cli::Sessions.generate_id
      Liaison::Cli::Sessions.snapshot(other, M::Session.new, "ollama-responses")
      Liaison::Cli::Sessions.snapshot(other, M::Session.new, "ollama")

      Liaison::Cli::Sessions.latest_deployment(other).should eq("ollama")
    end
  end
  ...
  it "keeps a burst of snapshots distinct and in order" do
    with_sandbox do
      id = Liaison::Cli::Sessions.generate_id
      written = (1..8).map { |n| Liaison::Cli::Sessions.snapshot(id, M::Session.new, "d#{n}") }

      files = Liaison::Cli::Sessions.snapshots(id)
      files.size.should eq(8)
      files.should eq(written.map { |path| File.basename(path) })
      files.map { |file| Liaison::Cli::Sessions.parse_snapshot(file)[0] }.uniq.size.should eq(8)
    end
  end
  ...
  it "returns nil for a snapshot with no deployment segment in its filename" do
    with_sandbox do
      ...
      id = Liaison::Cli::Sessions.generate_id
      dir = Liaison::Cli::Sessions.path_for(id)
      Dir.mkdir_p(dir)
      File.write(File.join(dir, "#{Time.utc.to_unix_ms}.json"), M::Archive.write(M::Session.new))

      Liaison::Cli::Sessions.latest_deployment(id).should be_nil
    end
  end
  ...
  it "raises, naming the session id, when asked for one that doesn't exist" do
    with_sandbox do
      expect_raises(Liaison::Cli::SessionError, /nonexistent-session/) do
        Liaison::Cli::Sessions.latest("nonexistent-session")
      end
    end
  end
  ...
  describe ".validate_id" do
    it "accepts the shapes people actually use" do
      %w[brisk-comet nightly_run_42 v2.1-tax-questions A1].each do |id|
        Liaison::Cli::Sessions.validate_id(id).should eq(id)
      end
    end
    ...
    it "refuses anything that would leave the sessions folder" do
      ["../escape", "a/b", "a\\b", "..", "nested/../..", ".hidden"].each do |id|
        expect_raises(Liaison::Cli::SessionError, /not a usable session id/) do
          Liaison::Cli::Sessions.validate_id(id)
        end
      end
    end

    it "refuses an empty or over-long id" do
      expect_raises(Liaison::Cli::SessionError) { Liaison::Cli::Sessions.validate_id("") }
      expect_raises(Liaison::Cli::SessionError) { Liaison::Cli::Sessions.validate_id("x" * 65) }
    end

    it "is enforced through path_for, so no verb can bypass it" do
      with_sandbox do
        expect_raises(Liaison::Cli::SessionError, /not a usable session id/) do
          Liaison::Cli::Sessions.path_for("../escape")
        end
      end
    end
  end
  ...
  it "falls back to a numbered name rather than failing when the space is full" do
    with_sandbox do
      Liaison::Cli::Sessions::ADJECTIVES.each do |adjective|
        Liaison::Cli::Sessions::NOUNS.each do |noun|
          Dir.mkdir_p(Liaison::Cli::Sessions.path_for("#{adjective}-#{noun}"))
        end
      end
