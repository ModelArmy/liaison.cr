require "../spec_helper"
require "../../src/liaison_cli/display"
...
describe Liaison::Cli::Display do
  off = Liaison::Cli::Defaults.new
  on = Liaison::Cli::Defaults.new(streaming: true, show_reasoning: true)

  describe "streaming" do
    it "stays off when nothing asked for it" do
      Liaison::Cli::Display.resolve(off, stdout: FakeTerminal.new).streaming?.should be_false
    end

    it "follows the config on a terminal" do
      Liaison::Cli::Display.resolve(on, stdout: FakeTerminal.new).streaming?.should be_true
    end
    ...
    it "declines to stream into something that is not a terminal, config or not" do
      Liaison::Cli::Display.resolve(on, stdout: a_pipe).streaming?.should be_false
    end
    ...
    it "streams into a pipe when the flag says so" do
      Liaison::Cli::Display.resolve(off, stream: true, stdout: a_pipe).streaming?.should be_true
    end

    it "lets --no-stream beat a config that asked for it" do
      Liaison::Cli::Display.resolve(on, stream: false, stdout: FakeTerminal.new).streaming?.should be_false
    end
  end

  describe "reasoning" do
    it "stays off by default" do
      Liaison::Cli::Display.resolve(off, stdout: FakeTerminal.new).show_reasoning?.should be_false
    end

    it "follows the config" do
      Liaison::Cli::Display.resolve(on, stdout: FakeTerminal.new).show_reasoning?.should be_true
    end

    it "takes the flag over the config, in both directions" do
      Liaison::Cli::Display.resolve(off, show_reasoning: true, stdout: a_pipe).show_reasoning?.should be_true
      Liaison::Cli::Display.resolve(on, show_reasoning: false, stdout: a_pipe).show_reasoning?.should be_false
    end
    ...
    it "shows reasoning into a pipe when asked, unlike streaming" do
      display = Liaison::Cli::Display.resolve(on, stdout: a_pipe)
