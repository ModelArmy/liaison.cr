...
module Liaison::Cli::Commands
  module Start
    extend self

    USAGE = "usage: liaison start <deployment> <prompt...> [--id <session-id>] " \
    ...
    # `--id` is for someone driving `liaison` from a script, who wants the
    ...
    private def claim(id : String) : String
      ...
      if Sessions.exists?(id)
        raise SessionError.new(
          "session #{id.inspect} already exists — use 'liaison continue #{id} ...' to add to it, " \
          "or pick another --id")
      end
      id
    end
