...
module Liaison::Cli
  ...
  # `liaison.yaml` written today keeps meaning what it meant.
  struct Defaults
  ...
  class Config
    ...
    # `$CWD/liaison.yaml`, then `$HOME/liaison.yaml`. See `docs/CLI_DESIGN.md`.
    def self.load : Config
      path = locate || raise ConfigError.new(
        "no liaison.yaml found via $LIAISON_CONFIG, #{Dir.current}, or $HOME — see docs/CLI_DESIGN.md")
      from_yaml(File.read(path))
    end

    # Explicit beats implicit: `$LIAISON_CONFIG`, if set, names the file
    # directly and skips the search — the same reasoning `$LIAISON_HOME`
    ...
    # real `liaison.yaml` an actual invocation happens to have left sitting in
    ...
    def self.locate : String?
      ...
      cwd_candidate = File.join(Dir.current, "liaison.yaml")
      return cwd_candidate if File.exists?(cwd_candidate)

      if home = ENV["HOME"]?
        home_candidate = File.join(home, "liaison.yaml")
    ...
    def self.from_yaml(text : String) : Config
      ...
      servers_node = root["servers"]?.try(&.as_h?) ||
                     raise ConfigError.new(legacy?(root) ? LEGACY_HELP : "liaison.yaml has no 'servers' key")
      ...
      table = root["deployments"]?.try(&.as_h?) || raise ConfigError.new("liaison.yaml has no 'deployments' key")
    ...
    LEGACY_HELP = "liaison.yaml has no 'servers' key, but its deployments carry inline server URLs — " \
    ...
    def deployment(name : String) : Deployment
      deployments[name]? || raise ConfigError.new(
        "no deployment named #{name.inspect} in liaison.yaml — have: #{deployments.keys.join(", ")}")
    end
