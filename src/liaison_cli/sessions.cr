...
module Liaison::Cli
  ...
  module Sessions
    ...
    # `$LIAISON_HOME` first if set — explicit beats implicit, the same
    # reasoning `Config` now applies to `$LIAISON_CONFIG`. This is a change
    # from the original order (`$CWD`, then `$HOME`, then `$LIAISON_HOME` as
    # a last resort): a real invocation's `.liaison` sitting in `$CWD` or
    ...
    def root : String
      return ENV["LIAISON_HOME"] if ENV["LIAISON_HOME"]?

      cwd_candidate = File.join(Dir.current, ".liaison")
      return cwd_candidate if Dir.exists?(cwd_candidate)

      if home = ENV["HOME"]?
        return File.join(home, ".liaison")
      end

      raise SessionError.new("no $HOME and no $LIAISON_HOME — nowhere to store sessions")
    end
    ...
    # gateway) — this fact belongs to *how you're using liaison*, not to the
